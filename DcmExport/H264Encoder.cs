...
namespace DcmExport
{
    public unsafe class H264Encoder:IDisposable
    {
        ...
        public readonly AVCodec* _pCodec;
        public readonly AVCodecContext* _pCodecContext;
        private readonly int _uSize;
        private readonly int _ySize;
        private Action<IntPtr, int, int, int, int,long> compressCallback;
        public H264Encoder(int fps, Size frameSize, Action<IntPtr, int, int, int, int,long> callback)
        {
          ...
          //  ffmpeg.av_hwdevice_ctx_create(&_pCodecContext->hw_device_ctx, AVHWDeviceType.AV_HWDEVICE_TYPE_DXVA2, null, null, 0).ThrowExceptionIfError();
        ...
        public void Encode(AVFrame frame)
        {
           ...
           if (frame.data[2] - frame.data[1] < _uSize) throw new ArgumentException("Invalid U data size.", nameof(frame));
            ...
            try
            {
                ...
                compressCallback?.Invoke((IntPtr)pPacket->data, pPacket->size, frame.width, frame.height, pPacket->flags ,pPacket->dts);
                //if (pPacket->flags == 1)
                //{
